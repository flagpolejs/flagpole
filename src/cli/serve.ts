...
const possibleEnvironments = ['dev', 'stag', 'prod', 'qa', 'rc', 'preprod', 'alpha', 'beta'];
...
const routes = {
    ...
    'POST /import/suite': (url: URL, response: http.ServerResponse, postData: any) => {
    ...
    'POST /add/scenario': (url: URL, response: http.ServerResponse, postData: any) => {
        if (!postData.suite || !postData.description || !postData.path || !postData.type) {
    ...
    'POST /add/suite': (url: URL, response: http.ServerResponse, postData: any) => {
        // Verify base data exists
        if (!postData.suiteName || !postData.suiteDescription ||
            !postData.scenarioDescription || !postData.scenarioPath || !postData.scenarioType
        ) {
    ...
    'GET /suite': (url: URL, response: http.ServerResponse) => {
        const suite: SuiteConfig | void = getSuite(response, url.searchParams.get('name'));
        if (suite) {
    ...
    'POST /suite': (url: URL, response: http.ServerResponse, postData: any) => {
        ...
        if (suite) {
            ...
            String(postData.tags).split(',').forEach(tag => {
    ...
    'POST /add/env': (url: URL, response: http.ServerResponse, postData: any) => {
        ...
        if (envName) {
            addEnv(response, new EnvConfig(Cli.config, { name: envName, defaultDomain: defaultDomain }));
        }
    },
...
const getSuite = (response: http.ServerResponse, suiteName: string | null): SuiteConfig | void => {
    if (!suiteName) {
...
const getTemplate = (httpResponse: http.ServerResponse): WebResponse => {
    const response: WebResponse = WebResponse.createFromTemplate(httpResponse, `${__dirname}/report.html`);
    ...
    let output: string = `
        ...
        <form method="POST" action="/init" id="frm">
            <div class="field">
                ...
                <input type="text" name="projectName" id="name" value="${process.cwd().split('/').pop()}">
            </div>
            <div class="field">
...
const sendGetEditSuite = (response: http.ServerResponse, suite: SuiteConfig): void => {
    let output: string = `
        <h2>Edit Suite</h2>
...
const sendGetAddScenario = (response: http.ServerResponse, suite: SuiteConfig) => {
    let output: string = `
        <h2>Add Scenario</h2>
...
const removeSuite = (response: http.ServerResponse, suiteName: string): void => {
...
const addSuite = (response: http.ServerResponse, opts: iSuiteOpts): void => {
    Cli.addSuite(opts)
        .then(() => {
            sendOutput(response, `Added new suite <em>${opts.suiteName}</em>. <a href="/">Back</a>`);
        })
        .catch((err) => {
    ...
    suites.forEach((suite: SuiteConfig) => {
        output += `
                    <tr>
                        <td>${suite.name}</td>
                        <td>
                            ${suite.tags.length ? suite.tags.join(', ') : '<em>no tags</em>'}
                        </td>
                        <td>
