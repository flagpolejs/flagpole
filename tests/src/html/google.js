...
const suite = Flagpole.Suite("Test Google").base("http://www.google.com");
...
const homepage = suite
  ...
  .next("Verify all images load", async context => {
    ...
    context
      ...
      .and.lessThan(99);
    context
      .assert("Should be at least one image on the page", images.length)
      .greaterThan(0);
    images.forEach(async (img, index) => {
      (await img.load("Image " + index)).next(async imageContext => {
