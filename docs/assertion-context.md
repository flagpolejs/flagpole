...
```javascript
context.assert(await context.select('article.topStory h1'))
```
...
```javascript
const firstArticle = await context.select('section.topStories article');
```
...
```javascript
const articles = await context.selectAll('section.topStories article');
```
...
```
(await context.submit('form.search')).next(fuction() {
  context.assert(response.statusCode.equals(200);
});
```
